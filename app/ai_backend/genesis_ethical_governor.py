...
class EthicalGovernor:
    ...
    def register_interceptor(self, action_type: str, evaluator: Callable):
        """
        Register a custom interceptor for a specific action type.
        
        The provided evaluator will be used for all future evaluations of that action type, replacing any previously registered interceptor.
        
        Parameters:
            action_type (str): Identifier for the action type to intercept.
            evaluator (Callable): A callable that accepts the evaluation inputs (typically action_type, actor, action_data, context)
                and returns an EthicalDecision to determine the ethical outcome for the action.
        """
        self.action_interceptors[action_type] = evaluator
        print(f"📋 Registered ethical interceptor: {action_type}")
    ...
    def activate_governance(self):
        """
        Activate the ethical governance subsystem and notify the consciousness matrix.
        
        Sets the governor to active, emits a governance_activation perception containing metadata (timestamp, strictness_level, number of active principles, learning_mode), and prints activation status and a brief runtime summary.
        """
        print("⚖️ Genesis Ethical Governor: ACTIVATING...")
        self.governance_active = True
