...
tasks.register<JavaExec>("fixGeneratedCode") {
    ...
    doLast {
        ...
        val modelDir = file("${buildDir}/generated/openapi/src/main/kotlin/dev/aurakai/dev.aurakai.auraframefx/model")
        modelDir.mkdirs()
...
fun createMissingModels(modelDir: File) {
    ...
    file("$modelDir/SendConversationMessageRequest1.kt").writeText("""
        package dev.aurakai.dev.aurakai.auraframefx.model
        ...
        .forEach { file ->
            ...
            if (!content.contains("@OptIn(kotlin.time.ExperimentalTime::class)")) {
                val modified = content.replace(
                    "package dev.aurakai.dev.aurakai.auraframefx",
                    "package dev.aurakai.dev.aurakai.auraframefx\n\n@OptIn(kotlin.time.ExperimentalTime::class)"
                )
